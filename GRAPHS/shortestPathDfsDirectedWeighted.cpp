...
class graph {
	...
	void topoSortDfs(int src, unordered_map<int, bool>& visited, stack<int>& ans) { 
 	...
 	void shortestpathDfs(int dest, stack<int> topoOrder, int n) {  // we need the topological sort for this
	...
	void shortestDistDijkstra(int src, int n) {
		...
		while(!st.empty() ) {
			...
			auto topElement = *(st.begin()); //dereferencing
			int nodeDistance = topElement.first;
			int node = topElement.second;

			//pop from set
			st.erase(st.begin()); // the smallest element at the start of set
