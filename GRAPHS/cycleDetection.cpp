...
public:
    ...
    bool detectCycleDfsDirected(int src, unordered_map<int,bool>&visited,unordered_map<int,bool>&dfsVisited)
{
	visited[src]=true;
	dfsVisited[src]=true;
	...
	for(auto nbr : adjList[src])
	{
		if(!visited[nbr])
		{
			return detectCycleDfsDirected(nbr,visited,dfsVisited);
		}
		if(visited[nbr] == true and dfsVisited[nbr]==true)
		{
			return true;
		}

		dfsVisited[nbr]=false;
		return false;
	}
}
};
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (!visited[i])
        {
            ans = g.detectCycleDfsDirected(i, visited, dfsVisited);
            if (ans == true)
                break;
        }
    }
